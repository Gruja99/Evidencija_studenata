...
import mysql.connector #connect to the MySQL server
...
konekcija = mysql.connector.connect(
    passwd="", user="root", database="evidencija_studenata1", port=3306, auth_plugin='mysql_native_password'
)
...
@app.route("/login", methods=["GET", "POST"]) #GET is used to request data from a specified resource
def login():
...
@app.route('/generacija2017')
def generacija():
    if ulogovan():
        upit ="SELECT * FROM p_studenti_17"
        kursor.execute(upit)
        studenti = kursor.fetchall()
...
def zavrsili():
    if ulogovan():
        upit ="SELECT * FROM p_zavrsili_17"
        kursor.execute(upit)
        studenti = kursor.fetchall()
...
def telefon():
    if ulogovan():
        upit ="""SELECT telefon(ime, prezime, broj_telefona) AS Kontakt FROM studenti"""
        kursor.execute(upit)
        studenti = kursor.fetchall()
      
        return render_template('telefon.html', tel_studenti = studenti)
    else:
        return redirect(url_for("login"))
...
def bodovi():
        if ulogovan():
             upit ="""SELECT username(email) AS Username, espb FROM studenti"""
             kursor.execute(upit)
             studenti = kursor.fetchall()
        
             return render_template('bodovi.html', bodovi_studenti = studenti)
        else:
             return redirect(url_for("login"))
...
def predmeti():
    if ulogovan():
         kursor.callproc('predmeti')
         for rez in kursor.stored_results():
            predmeti = rez.fetchall()    
...
def korisnici():
    if ulogovan():
        kursor.callproc('korisnici')
        for rez in kursor.stored_results():
            korisnici = rez.fetchall()    
...
def ocena_brisanje(student_id, ocena_id):
    if ulogovan():
        ...
        espb = kursor.fetchone()
        # Update tabele student
        upit = "UPDATE studenti SET espb=%s, prosek_ocena=%s WHERE student_id=%s"
        vrednosti = (espb['rezultat'], prosek_ocena['rezultat'], id)
        kursor.execute(upit, vrednosti)
        konekcija.commit()
